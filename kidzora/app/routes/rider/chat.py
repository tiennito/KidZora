...
from .utils import rider_bp, approved_rider_required
    ...
    try:
        ...
        if not seen:
            ...
            try:
                ur = supabase.table('profiles').select(
                    'id,first_name,last_name,business_name,role,avatar_url'
                ).neq('role', 'rider').order('first_name').execute()
                for p in (ur.data or []):
                    all_users.append({
                        'id':         p['id'],
                        ...
                        'chat_url':   url_for('rider.chat', other_user_id=p['id']),
                    })
            except Exception:
                pass
            return render_template('rider/inbox.html', convos=[], all_users=all_users,
                badge='Rider',
                dashboard_url=url_for('rider.dashboard'),
                tabs_config=[
        ...
        for oid, latest in seen.items():
            ...
            convos.append({
                ...
                'chat_url':   url_for('rider.chat', other_user_id=oid),
            })

        convos.sort(key=lambda c: c['latest_at'], reverse=True)

    except Exception as e:
        print(f"[rider.inbox] error: {e}")

    # Fetch all non-rider users so rider can start new conversations
    all_users = []
    try:
        ur = supabase.table('profiles').select(
            'id,first_name,last_name,business_name,role,avatar_url'
        ...
        print(f"[rider.inbox] all_users error: {e}")

    return render_template('rider/inbox.html', convos=convos, all_users=all_users,
        badge='Rider',
        dashboard_url=url_for('rider.dashboard'),
        tabs_config=[
            {'key': 'admin',  'label': 'Admin',   'icon': 'fa-shield-alt'},
...
@rider_bp.route('/chat/<other_user_id>')
@approved_rider_required
def chat(other_user_id):
    ...
    return render_template(
        ...
        messages=msgs,
        embed=embed,
        back_url=url_for('rider.inbox'),
        send_url=url_for('rider.chat_send',   other_user_id=other_user_id),
        poll_url=url_for('rider.chat_poll',   other_user_id=other_user_id),
        typing_url=url_for('rider.chat_typing', other_user_id=other_user_id),
    )


# ── Send message (AJAX POST) ──────────────────────────────────────────────────

@rider_bp.route('/chat/<other_user_id>/send', methods=['POST'])
@approved_rider_required
def chat_send(other_user_id):
    ...
    try:
        ...
        sender_name = f"{current_user.first_name} {current_user.last_name}".strip() or 'Rider'
        push(
        ...
        print(f"[rider.chat_send] error: {e}")
        return jsonify({'ok': False, 'error': str(e)}), 500


# ── Poll for new messages (AJAX GET) ─────────────────────────────────────────

@rider_bp.route('/chat/<other_user_id>/poll')
@approved_rider_required
...
@rider_bp.route('/chat/<other_user_id>/typing', methods=['POST'])
@approved_rider_required
