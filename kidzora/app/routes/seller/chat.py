    ...
    try:
        ...
        if not seen:
            ...
            try:
                ur = supabase.table('profiles').select(
                    'id,first_name,last_name,business_name,role,avatar_url'
                ).neq('role', 'seller').order('first_name').execute()
                for p in (ur.data or []):
                    all_users.append({
                        'id':         p['id'],
                        ...
                        'chat_url':   url_for('seller.chat', other_user_id=p['id']),
                    })
            except Exception:
                pass
            return render_template('seller/inbox.html', convos=[], all_users=all_users,
                badge='Seller',
                dashboard_url=url_for('seller.dashboard'),
                tabs_config=[
                    {'key': 'buyer',  'label': 'Buyers',  'icon': 'fa-user'},
                    {'key': 'rider',  'label': 'Riders',  'icon': 'fa-motorcycle'},
                    {'key': 'admin',  'label': 'Admin',   'icon': 'fa-shield-alt'},
                ],
                stat3_role='buyer', stat3_label='Buyers',
            )
        ...
        for oid, latest in seen.items():
            ...
            convos.append({
                ...
                'chat_url':   url_for('seller.chat', other_user_id=oid),
            })

        convos.sort(key=lambda c: c['latest_at'], reverse=True)

    except Exception as e:
        print(f"[seller.inbox] error: {e}")

    # Fetch all non-seller users so seller can start new conversations
    all_users = []
    try:
        ur = supabase.table('profiles').select(
            'id,first_name,last_name,business_name,role,avatar_url'
        ...
        print(f"[seller.inbox] all_users error: {e}")

    return render_template('seller/inbox.html', convos=convos, all_users=all_users,
        badge='Seller',
        dashboard_url=url_for('seller.dashboard'),
        tabs_config=[
            {'key': 'buyer',  'label': 'Buyers',  'icon': 'fa-user'},
...
@seller_bp.route('/chat/<other_user_id>')
@seller_required
def chat(other_user_id):
    ...
    return render_template(
        ...
        messages=msgs,
        embed=embed,
        back_url=url_for('seller.inbox'),
        send_url=url_for('seller.chat_send',   other_user_id=other_user_id),
        poll_url=url_for('seller.chat_poll',   other_user_id=other_user_id),
        typing_url=url_for('seller.chat_typing', other_user_id=other_user_id),
    )


# ── Send message (AJAX POST) ──────────────────────────────────────────────────

@seller_bp.route('/chat/<other_user_id>/send', methods=['POST'])
@seller_required
def chat_send(other_user_id):
    ...
    try:
        ...
        print(f"[seller.chat_send] error: {e}")
        return jsonify({'ok': False, 'error': str(e)}), 500


# ── Poll for new messages (AJAX GET) ─────────────────────────────────────────

@seller_bp.route('/chat/<other_user_id>/poll')
@seller_required
...
@seller_bp.route('/chat/<other_user_id>/typing', methods=['POST'])
@seller_required
